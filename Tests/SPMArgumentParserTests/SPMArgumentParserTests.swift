...
import XCTest
@testable import SPMArgumentParser
...
final class SPMArgumentParserTests: XCTestCase {
    func testBasics() throws {
        ...
        XCTAssertNil(args.get(revision))
        ...
        XCTAssertNil(try args.get("invalid") as Int?)
    ...
    func testBinderThrows() throws {
        ...
        binder.bind(option: parser.add(option: "--foo", kind: String.self), to: fillOptions)

        // Not throwing.
        let result = try parser.parse(["--foo", "nothrow"])
        var options = Options()
        XCTAssertNoThrow(try binder.fill(parseResult: result, into: &options))
    ...
    func testOptions() throws {
        ...
        binder.bind(positional: parser.add(positional: "package", kind: String.self),
                    to: { $0.package = $1 })

        binder.bindPositional(parser.add(positional: "foo", kind: String.self),
                              parser.add(positional: "bar", kind: Int.self),
                              to: { $0.foo = $1; $0.bar = $2 })

        binder.bind(option: parser.add(option: "--branch", shortName: "-b", kind: String.self),
                    to: { $0.branch = $1 })

        binder.bind(option: parser.add(option: "--verbose", kind: Bool.self),
                    to: { $0.isVerbose = $1 })

        binder.bindArray(option: parser.add(option: "-Xld", kind: [String].self),
                         to: { $0.xld = $1 })

        binder.bindArray(parser.add(option: "-xlinker", kind: [String].self),
