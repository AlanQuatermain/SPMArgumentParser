...
/// Errors which may be encountered when running the argument parser.
...
extension ArgumentConversionError: Equatable {
    public static func == (lhs: ArgumentConversionError, rhs: ArgumentConversionError) -> Bool {
        ...
        case (.unknown, _):
            return false
        case (.typeMismatch(let lv, let lt), .typeMismatch(let rv, let rt)):
            return lv == rv && lt == rt
        case (.typeMismatch, _):
            return false
...
public enum Shell: String, StringEnumArgument {
    ...
    public static var completion: ShellCompletion = .values([
        ...
        (fish.rawValue, "generate completion script for Fish shell"),
        ])
}
...
public enum ShellCompletion {
    ...
    /// Custom function for generating completions. Must be provided
    /// in the script's scope.
    case function(String)

    /// Offers completions from a predefined list. A description can
    /// be provided which is shown in some shells, like zsh.
    case values([(value: String, description: String)])
}

/// A protocol representing the possible types of arguments.
///
/// Conforming to this protocol will qualify the type to act as a
/// positional and optional argument in the argument parser.
...
///
/// Conforming to this protocol will automatically make an enum with its
...
public struct PathArgument: ArgumentKind {
    ...
    public init(argument: String) throws {
        ...
        if argument.hasPrefix("/") {
            self.pathURL = URL(fileURLWithPath: argument)
        } else {
            let cwd = URL(fileURLWithPath: FileManager.default.currentDirectoryPath, isDirectory: true)
            self.pathURL = cwd.appendingPathComponent(argument)
        }
    }
...
protocol ArgumentProtocol: Hashable {
    // FIXME: This should be constrained to ArgumentKind by Array can't conform
    // to it: `extension of type 'Array' with constraints cannot have an
    // inheritance clause`.
    //
    /// The argument kind of this argument for e.g. String, Bool, etc.
    ...
    /// for short names for e.g. `--help` -> `-h`.
...
public final class OptionArgument<Kind>: ArgumentProtocol {
    ...
    func parse(_ kind: ArgumentKind.Type,
               with parser: inout ArgumentParserProtocol) throws -> [ArgumentKind] {
        do {
    ...
    func _parse(_ kind: ArgumentKind.Type,
                with parser: inout ArgumentParserProtocol) throws -> [ArgumentKind] {
...
///
/// - Note: Only used for argument parsing purposes.
...
public protocol ArgumentParserProtocol {
    ...
    /// Provides (consumes) and returns the next argument. Returns `nil` if there
    /// ar no arguments left.
...
public final class ArgumentParser {
    ...
    public class Result: CustomStringConvertible {
        ...
        /// Result of the parent parser, if any.
        ...
        /// - throws: An ArgumentParserError.invalidValue error if the parsed argument does
        ...
        public func get<T>(_ name: String) throws -> T? {
            ...
            guard let typedValue = value as? T else {
                throw ArgumentParserError.invalidValue(
                    argument: name,
                    error: .typeMismatch(value: String(describing: value), expectedType: T.self)
                )
            }
            return typedValue
        }
    ...
    public func add<T: ArgumentKind>(
        ...
        assert(!optionArguments.contains(where: { $0.name == option }), "Cannot define an option twice")

        let argument = OptionArgument<T>(name: option, shortName: shortName, strategy: .oneByOne,
                                         usage: usage, completion: completion ?? T.completion)
    ...
    public func add<T: ArgumentKind>(
        ...
        let argument = OptionArgument<[T]>(name: option, shortName: shortName, strategy: strategy,
                                           usage: usage, completion: completion ?? T.completion)
        optionArguments.append(AnyArgument(argument))
        return argument
    }
    ...
    public func add<T: ArgumentKind>(
        ...
        let argument = PositionalArgument<T>(name: positional, strategy: .oneByOne, optional: optional,
                                             usage: usage, completion: completion ?? T.completion)
    ...
    public func add<T: ArgumentKind>(
        ...
        let argument = PositionalArgument<[T]>(name: positional, strategy: strategy, optional: optional,
                                               usage: usage, completion: completion ?? T.completion)
        positionalArguments.append(AnyArgument(argument))
        return argument
    }
    ...
    struct Parser: ArgumentParserProtocol {
        ...
        init(associatedArgumentValue: String?,
             argumentsIterator: IndexingIterator<[String]>,
    ...
    private func parse(_ arguments: [String] = [], parent: Result?) throws -> Result {
        ...
        while let argumentString = argumentsIterator.next() {
            ...
            if argumentString == "-h" || argumentString == "-help" || argumentString == "--help" {
                ...
                // If this parser has subparsers, we only allow one positional argukent.
                if !subparsers.isEmpty {
                    ...
                    // Parse rest of the arguments with the subparser.
                ...
                parser = Parser(associatedArgumentValue: nil, argumentsIterator: argumentsIterator, currentArgument: argumentString)
            } else {
                ...
                parser = Parser(associatedArgumentValue: value, argumentsIterator: argumentsIterator, currentArgument: argumentString)
            }
        ...
        // Report if there are any non-optional positional arguments left which were not
    ...
    public func printUsage(on stream: OutputByteStream) {
        ...
        let maxWidth: Int
        // Determine the max width vbased on argument length or choose the
        // default width if max width is longer.
...
public final class ArgumentBinder<Options> {
    /// The signature of the body closure.
    ...
    public func bind<T>(option: OptionArgument<T>, to body: @escaping (inout Options, T) throws -> Void) {
        addBody {
    ...
    /// Bind an array option argukent.
    public func bindArray<T>(option: OptionArgument<[T]>, to body: @escaping (inout Options, [T]) throws -> Void) {
        addBody {
            guard let result = $1.get(option) else { return }
            try body(&$0, result)
        }
    ...
    /// B ind a positional argument.
    public func bind<T>(positional: PositionalArgument<T>, to body: @escaping (inout Options, T) throws -> Void) {
        addBody {
    ...
    public func bindArray<T>(positional: PositionalArgument<[T]>, to body: @escaping (inout Options, [T]) throws -> Void) {
        addBody {
            // All the positional arguments will always be present.
            guard let result = $1.get(positional) else { return }
            try body(&$0, result)
    ...
    public func bindPositional<T, U>(_ first: PositionalArgument<T>, _ second: PositionalArgument<U>,
    ...
    public func bindPositional<T, U, V>(_ first: PositionalArgument<T>, _ second: PositionalArgument<U>,
    ...
    /// Bind two options.
    public func bind<T, U>(_ first: OptionArgument<T>, _ second: OptionArgument<U>,
                           to body: @escaping (inout Options, T?, U?) throws -> Void) {
        addBody {
    ...
    /// Bind three options.
    public func bind<T, U, V>(_ first: OptionArgument<T>, _ second: OptionArgument<U>, _ third: OptionArgument<V>,
                              to body: @escaping (inout Options, T?, U?, V?) throws -> Void) {
        addBody {
    ...
    public func bindArray<T, U>(_ first: OptionArgument<[T]>, _ second: OptionArgument<[U]>,
                                to body: @escaping (inout Options, [T], [U]) throws -> Void) {
        addBody {
            try body(&$0, $1.get(first) ?? [], $1.get(second) ?? [])
        }
    ...
    /// Bind three array options.
    public func bindArray<T, U, V>(_ first: OptionArgument<[T]>, _ second: OptionArgument<[U]>, _ third: OptionArgument<[V]>,
                                   to body: @escaping (inout Options, [T], [U], [V]) throws -> Void) {
        addBody {
            try body(&$0, $1.get(first) ?? [], $1.get(second) ?? [], $1.get(third) ?? [])
        }
    ...
    /// Bind a subparser.
    public func bind(parser: ArgumentParser, to body: @escaping (inout Options, String) throws -> Void) {
        addBody {
...
fileprivate extension String {
    ...
    /// Trims whitespace from both ends of a string. If the resulting string is empty, returns
    /// `nil`.
    ///
    /// Useful because you can short-circuit the result and thus handle "falsy" strings
    /// in an elegant way:
    ///
    ...
    /// If delimiter is not found, the second substring is nil.
...
fileprivate extension Collection where Element : Equatable {
    ...
    func split(around delimiter: [Element]) -> ([Element], [Element]?) {
        let orig = Array(self)
